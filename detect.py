...
def main():
    ...
    # Use Daimler detector because it is trained to recognise people 48x96 pixels(1:2 ratio to make calculations easier)
    ...
    while True:
        ...
        orig = image.copy()
        s=int(frame.shape[1]*1.4)
        pitch=imutils.resize(pitch,width=s,height=720)
        

        # Line detection: gray image / fill lines / canny / HoughLinesP
        grayImg = cv2.cvtColor(orig,cv2.COLOR_BGR2GRAY)
        blurredGrayImg = cv2.GaussianBlur(grayImg, (15, 15), 0)#Removes high frequency noise
        ...
        if (offsideLine is not None and len(offsideLine) > 0):
            lineX = (offsideLine[3] - offsideLine[1]) / (offsideLine[2] - offsideLine[0]) #y2-y1/x2-x1
            lineB =  offsideLine[1] - lineX*offsideLine[0] 
            t1y = 0
            t1x = int(lineB)
            t2y = int(image.shape[1])
            t2x = int(lineX*image.shape[1] + lineB)
            #cv2.line(image, (t1x, t1y), (t2x, t2y), (255, 0, 0), 3)
            ...
            if (weights[i] > 0.7):
                ...
                if (percGreen < 0.7 and percGreen > 0.1):
                    ...
                    if (percTeam1 > 0.03):
                        ...
                        cv2.rectangle(pitch,(x,y),(x+4,y+4),(0,0,255),2)

                        # Calculate offside line from found player(from intersection point to the last player of defense line   
                        if (box5Line is not None and len(box5Line) > 0):
                            ...
                            #cv2.line(image, (ro1x, ro1y), (ro2x, ro2y), (255, 0, 0), 3)
                        ...
                        cv2.rectangle(pitch,(x,y),(x+4,y+4),(0,255,0),2)
        ...
        pitch=imutils.resize(pitch,width=720)
        pitch=imutils.resize(pitch,height=720)
